...
<body>
            ...
            <div class="panel">
                ...
                <div class="preset-boards">
                    ...
                    <div class="preset-board" data-preset="advanced">
                        <h4>进阶板型</h4>
                        <p>4狼人、3村民、预言家、女巫、猎人、白狼王</p>
                    </div>
                    <div class="preset-board" data-preset="mechanical">
                        <h4>机械狼板型</h4>
                        <p>3狼人、机械狼、4村民、预言家、女巫、守卫</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let roles = [
            ...
            { id: 'seer', name: '预言家', icon: 'fas fa-eye', count: 0, description: '夜晚可以查验一名玩家的身份', side: 'village', custom: false },
            { id: 'witch', name: '女巫', icon: 'fas fa-flask', count: 0, description: '拥有一瓶解药和一瓶毒药', side: 'village', custom: false },
            { id: 'hunter', name: '猎人', icon: 'fas fa-crosshairs', count: 0, description: '死亡时可以开枪带走一名玩家', side: 'village', custom: false },
            { id: 'guard', name: '守卫', icon: 'fas fa-shield-alt', count: 0, description: '每晚可以守护一名玩家免受狼人袭击', side: 'village', custom: false },
            ...
            { id: 'knight', name: '骑士', icon: 'fas fa-horse', count: 0, description: '可以在白天决斗一名玩家，若对方是狼人则将其杀死', side: 'village', custom: false },
            { id: 'cupid', name: '丘比特', icon: 'fas fa-heart', count: 0, description: '第一夜指定两名玩家成为情侣', side: 'village', custom: false },
            { id: 'mechanical_wolf', name: '机械狼', icon: 'fas fa-robot', count: 0, description: '第一夜学习一名玩家的身份，之后获得该身份的技能', side: 'wolf', custom: true }
        ];
        ...
        const gamePhases = [
            ...
            { 
                title: "第一夜", 
                description: "第一夜行动顺序：丘比特 → 狼人 → 机械狼 → 女巫 → 预言家 → 守卫",
                actions: ["请按照夜晚行动顺序依次行动"]
            },
            { 
                title: "第一天白天", 
                description: "天亮了，所有人请睁眼。宣布昨晚的死亡情况。",
                actions: ["天亮了，所有人请睁眼", "宣布昨晚的死亡信息", "请死者发表遗言"]
            },
            { 
                title: "第二天夜晚", 
                description: "第二夜行动顺序：狼人 → 女巫 → 预言家 → 守卫",
                actions: ["请按照夜晚行动顺序依次行动"]
            },
            { 
                title: "第二天白天", 
            ...
            { 
                title: "第三天夜晚", 
                description: "第三夜行动顺序：狼人 → 女巫 → 预言家 → 守卫",
                actions: ["请按照夜晚行动顺序依次行动"]
            },
            { 
                title: "第三天白天", 
        ...
        // 游戏日志
        let gameLog = [
            { phase: "游戏开始", content: "游戏开始，共12名玩家", time: "10:00" },
            { phase: "板型配置", content: "使用标准板型：4狼人、4村民、预言家、女巫、猎人、守卫", time: "10:02" }
        ];
        ...
        function updatePlayerList() {
            ...
            if (players.length === 0) {
                ...
                // 如果游戏已开始，自动分配身份
                if (gameStarted && currentBoard.length > 0) {
        ...
        function startGame() {
            ...
            if (currentBoard.length < 6) {
                if (!confirm(`当前板型只有 ${currentBoard.length} 个角色，建议至少6人开始游戏。确定开始吗？`)) {
                    return;
                }
            }
            ...
            // 添加到游戏日志
            addLogEntry(`游戏开始，玩家数量: ${playerCount}`, "游戏开始");
        }
        ...
        function updateGamePhase() {
            ...
            gamePhaseEl.innerHTML = `
                <div class="phase-title">${phase.title} (${currentPhaseIndex + 1}/${gamePhases.length})</div>
                <div class="phase-description">
                    ${phase.description}
                    ${actionsHTML}
                </div>
            ...
            // 添加到游戏日志
            addLogEntry(`进入阶段: ${phase.title}`, "阶段切换");
        }
        ...
        function loadPresetBoard(presetId) {
            ...
            switch(presetId) {
                case 'standard':
                    ...
                    break;
                case 'advanced':
                    // 进阶板型: 4狼人、3村民、预言家、女巫、猎人、白狼王
                    presetRoles = [
                        'werewolf', 'werewolf', 'werewolf', 'werewolf',
                        'villager', 'villager', 'villager',
                        'seer', 'witch', 'hunter', 'whitewolf'
                    ];
                    break;
                case 'mechanical':
                    // 机械狼板型: 3狼人、机械狼、4村民、预言家、女巫、守卫
                    presetRoles = [
            ...
            const presetName = presetId === 'standard' ? '标准' : presetId === 'advanced' ? '进阶' : '机械狼';
